...
# x, y 설정
x = parch_counts.index.astype(str) # 데이터프레임의 인덱스를 문자여로 반환
...
# 첫 번째 서브플롯 설정(선 그래프)
axes[0].plot(x, y1, '-o', color='indigo', markersize=7, linewidth=3, alpha=0.7, lable='Not Survived')
...
# .twinx() 함수를 사용하여 두 번째 그래프의 축을 공유하는 새로운 Axes 객체 생성
ax2 = ax1.twinx()
...
ax2.tick_params(axis='upper right', bbox_to_anchor=(1, 0.9))
...
# 히스토그램 그리기
plt.hist(titanic['Age'], bins=20, color='skyblue', edgecolor)