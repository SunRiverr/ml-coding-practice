...
# map 함숮를 이용하여 rating 컬럼의 값을 딕셔너리를 기반으로 변환하여 age_group 컬럼에 저장
...
plt.title('We see more movies than TV shows in Netflix.', fontfamily='serif', fontsize=12)
plt.show()
...
# title열의 값이 ‘Sankofa’인 행 전체를 확인하여 county 열과 age_group 열의 값이 어떻게 이루어져 있는지 확인
...
plt.title('Here we see interesting differences between countries. Most shows in')